...
const MyPayBills = () => {
  ...
  const confirmDelete = async () => {
    try {
      await fetch(`http://localhost:2000/myBills/${deleteId}`, { method: "DELETE" });
      toast.success("Deleted successfully!");
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 relative">
      <h2 className="text-3xl font-bold text-center mb-6">
        <span className="text-color2">My Paid</span> <span className="text-color1">Bills</span>
      </h2>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
          <div className="bg-base-300 p-6 rounded-lg w-full max-w-md relative">
            <button onClick={() => setModalOpen(false)}
              className="absolute top-2 right-2 font-bold text-xl">✕</button>
            ...
            <form onSubmit={handleUpdate} className="space-y-2">
              ...
              <input type="date" className="input border p-2 w-full rounded"
                name="date" value={formData.date || ""} onChange={handleChange} required />
              <input type="number" className="input border p-2 w-full rounded"
                name="amount" value={formData.amount || ""} onChange={handleChange} required />

              <button className="w-full py-2 submit-btn rounded-xl" disabled={updating}>
                {updating ? "Updating..." : "Update"}
              </button>
            </form>
          </div>
